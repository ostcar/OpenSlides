...
def init_builtin_workflows():
    """
    Saves a simple an a complex workflow into the database. This function is only called manually.
    """
    ...
    state_2_2 = State(name=ugettext_noop('permitted'), action_word=ugettext_noop('permit'),
                      allow_create_poll=True, allow_submitter_edit=True, dont_set_new_version_active=True)
    state_2_3 = State(name=ugettext_noop('accepted'), action_word=ugettext_noop('accept'))
    state_2_4 = State(name=ugettext_noop('rejected'), action_word=ugettext_noop('reject'))
    state_2_5 = State(name=ugettext_noop('withdrawed'), action_word=ugettext_noop('withdraw'))
    state_2_6 = State(name=ugettext_noop('adjourned'), action_word=ugettext_noop('adjourn'))
    state_2_7 = State(name=ugettext_noop('not concerned'))
    state_2_8 = State(name=ugettext_noop('commited a bill'), action_word=ugettext_noop('commit a bill'))
    state_2_9 = State(name=ugettext_noop('needs review'))
    state_2_10 = State(name=ugettext_noop('rejected (not authorized)'), action_word=ugettext_noop('reject (not authorized)'))
    state_2_1.save()
    state_2_2.save()
    state_2_3.save()
    state_2_4.save()
